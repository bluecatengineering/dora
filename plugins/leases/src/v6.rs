//! Stateful DHCPv6 lease handling for clustered mode.
//!
...
impl fmt::Display for V6LeaseKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "(subnet={}, duid={}, iaid={})", self.subnet, self.duid, self.iaid)
    }
}
...
// ---------------------------------------------------------------------------
// ClusteredV6Leases plugin (T028)
// ---------------------------------------------------------------------------

/// Clustered-mode stateful DHCPv6 lease plugin.
///
...
impl fmt::Debug for ClusteredV6Leases {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ClusteredV6Leases")
            .field("server_id", &self.server_id)
            .finish()
    }
}

impl ClusteredV6Leases {
    pub fn new(
        cfg: Arc<DhcpConfig>,
        coordinator: LeaseCoordinator,
        server_id: String,
    ...
    fn get_v6_network<'a>(&'a self, ctx: &MsgContext<v6::Message>) -> Option<&'a config::v6::Network> {
        let meta = ctx.meta();
    ...
    async fn handle_solicit(
        ...
        // we'd use an IP manager. For v6 clustered mode, we coordinate via NATS.
        ...
        match self.coordinator.reserve(record).await {
            ...
            Ok(LeaseOutcome::Conflict { expected_revision, actual_revision }) => {
    ...
    async fn handle_request_renew(
        ...
        // Coordinate with NATS
        let record = self.make_v6_lease_record(
            requested_ip,
            &key,
            expires_at,
            LeaseState::Leased,
...
#[async_trait]
impl Plugin<v6::Message> for ClusteredV6Leases {
    ...
    async fn handle(&self, ctx: &mut MsgContext<v6::Message>) -> Result<Action> {
        ...
        match msg_type {
            ...
            _ => {
                ...
                debug!(?msg_type, "v6 leases plugin: non-stateful msg type, continuing");
                Ok(Action::Continue)
            }
        }
    }
...
impl dora_core::Register<v6::Message> for ClusteredV6Leases {
    fn register(self, srv: &mut dora_core::Server<v6::Message>) {
        info!("ClusteredV6Leases plugin registered");
...
mod tests {
    ...
    fn test_extract_v6_lease_key_empty_duid() {
        let mut msg = v6::Message::new(v6::MessageType::Solicit);
        msg.opts_mut()
            .insert(v6::DhcpOption::ClientId(vec![])); // empty DUID
        let iana = v6::IANA {
            id: 1,
            t1: 3600,
            t2: 5400,
    ...
    fn test_known_lease_cache_operations() {
        ...
        assert_eq!(lease.unwrap().ip, "2001:db8::100".parse::<Ipv6Addr>().unwrap());
